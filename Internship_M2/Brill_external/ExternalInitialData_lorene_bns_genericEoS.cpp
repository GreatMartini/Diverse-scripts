...
void SAMRAI_External_Data::external_loadData(int vars, ...) {
  ...
  printf("$$ lorene_binary_neutron_stars: grid size nx=%d, ny=%d, nz=%d\n",
                      nx,ny,nz);
  ...
  for (int k=0; k<nz; k++) {
    for (int j=0; j<ny; j++) {
      for (int i=0; i<nx; i++) {
        ...
        if (*eosType == 0) {
          if (rho[i3D] < (*vacuum_rho)) rho[i3D] = (*vacuum_rho);
          if (eps[i3D]   < (*vacuum_rho)) eps[i3D] = (*vacuum_rho);
  ...
  magneticField(P, Sx, Sy, Sz, Bx, By, Bz, gxx, gxy, gxz, gyy, gyz, gzz, *tov_angle, *tov_Asize, *tov_facp, *vacuum_rho, r1, r2, dist, init_domain_x, init_domain_y, init_domain_z, init_patch_x, init_patch_y, init_patch_z, d_ghost_width, dx, nx, ny, nz);

  adm2bssn(DYf, Ye, eps, tau, rho, D, vx, 
           vy, vz, Sx, Sy, Sz, 
           P, Bx, By, Bz, gxx, 
           gxy, gxz, gyy, gyz, gzz,
           Kxx, Kxy, Kxz, Kyy, Kyz, 
  ...
  for (int k=0; k<nz; k++) {
    for (int j=0; j<ny; j++) {
      for (int i=0; i<nx; i++) {
        ...
        if (r1sq <= t1) {
          Sx[i3D]  = -(y_fd-ys1)*tov_Asize*max(P[i3D]-tov_facp*vacuum, 0.0)*max(P[i3D]-tov_facp*vacuum, 0.0);
          Sy[i3D]  =  (x_fd-xs1)*tov_Asize*max(P[i3D]-tov_facp*vacuum, 0.0)*max(P[i3D]-tov_facp*vacuum, 0.0);
          Sz[i3D]  = 0.0;
        }
        if (r2sq <= t2) {
            Sx[i3D]  = -(y_fd-ys2)*tov_Asize*max(P[i3D]-tov_facp*vacuum, 0.0)*max(P[i3D]-tov_facp*vacuum, 0.0);
            Sy[i3D]  =  (x_fd-xs2)*tov_Asize*max(P[i3D]-tov_facp*vacuum, 0.0)*max(P[i3D]-tov_facp*vacuum, 0.0);
            Sz[i3D]  = 0.0;
        }
        if (r1sq > t1 && r2sq > t2) {
          Sx[i3D]  = 0.0;
...
void SAMRAI_External_Data::adm2bssn(double *DYf, double *Ye, double *eps, double *tau, double *rho, double *D,   double *vx,     
