...
#ifndef included_appu_SlicerDataWriter
#define included_appu_SlicerDataWriter
...
private:
    ...
    */
   static const int SLICER_MASTER;

   /*
    * Static integer constant describing MPI message tag.
    */
   static const int SLICER_FILE_CLUSTER_WRITE_BATON;
   ...
   struct patchExtentsStruct {
      int lower[SLICER_FIXED_DIM];
      int upper[SLICER_FIXED_DIM];
      double xlo[SLICER_FIXED_DIM];
      double xhi[SLICER_FIXED_DIM];
   };
    ...
    */
   enum variable_type { SLICER_SCALAR = 0,
                        SLICER_VECTOR = 1,
                        SLICER_TENSOR = 2 };
    ...
    */
   enum variable_data_type { SLICER_INT = 3,
                             SLICER_FLOAT = 4,
                             SLICER_DOUBLE = 5,
                             SLICER_DATA_TYPE_BAD = 990 };
    ...
    */
   enum variable_centering { SLICER_CELL = 6,
                             SLICER_NODE = 7,
                             SLICER_UNKNOWN_CELL = 8,
                             SLICER_UNKNOWN_NODE = 9,
                             SLICER_CENTERING_BAD = 991 };
    ...
    */
   enum grid_type { SLICER_CARTESIAN = 10,
                    SLICER_DEFORMED = 11 };
   ...
   struct VisItItem {
      ...
      struct patchMinMaxStruct *
      d_master_min_max[SLICER_MAX_NUMBER_COMPONENTS];
   ...
   /*
    * Filters the MinMax structure with patches which have data.
    */
    patchMinMaxStruct*
    filterMinMaxStructArray(patchMinMaxStruct* all_patches_min_max, int total_patches, int total_patches_with_data) ;

    /*
     * Obtains the patch index for a global patch number
     */
    int 
    globalPatchNumberToIndex(int globalId);
    ...
    * Slice information
    */
    int d_plane_normal_axis;
    float d_distance_to_origin;
    std::vector<int> d_num_patches_per_level;
    std::vector<bool> d_patch_with_data;
    int d_total_number_patches_with_data;
   ...
   initializeCallback()
   {
      t_write_plot_data = tbox::TimerManager::getManager()->getTimer(
         "appu:SlicerDataWriter::writePlotData()");
   }
